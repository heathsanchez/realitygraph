...
from realitygraph.abgp.freeze_review import audit_g_exchangeability_design
from realitygraph.abgp.runner import _b_inputs
from realitygraph.abgp.qualification import run_qualification


class HonestControlTests(unittest.TestCase):
    def exchange_api(self):
        name = 'realitygraph.abgp.exchangeability'
    ...
    def test_b_representation_digest_is_stable_across_process_hash_seeds(self):
        code = "from realitygraph.abgp.arm_b import _canonical_digest; print(_canonical_digest(frozenset(str(i) for i in range(20))))"
