...
from openml_v7 import (
    sampled_indices,split_indices,numeric_matrix,calibration_evaluation,compile_model,MIN_GAIN
)
...
from probe_openml_regression_transfer_v1_manifest import main as freeze_manifest
...
def training_median_binary_target(y_values,train):
    vals=[finite_float(y_values[i]) for i in train]
    if any(v is None for v in vals):
        raise ValueError("non-finite regression target in training split")
    ordered=sorted(vals)
    n=len(ordered)
    threshold=ordered[n//2] if n%2 else (ordered[n//2-1]+ordered[n//2])/2.0
    labels=[]
    for value in y_values:
        x=finite_float(value)
        if x is None:
            raise ValueError("non-finite regression target in sampled rows")
        labels.append(1 if x>threshold else 0)
    if len({labels[i] for i in train})<2:
        raise ValueError("train-median binary consequence collapsed")
    return threshold,labels

def load_regression_world(task_id,data_id,expected_name,policy,budget_policy):
    task=openml.tasks.get_task(task_id,download_splits=False)
    dataset=task.get_dataset()
    if int(dataset.dataset_id)!=data_id:
        raise AssertionError("OpenML regression task/data id drift")
    if str(dataset.name)!=expected_name:
        raise AssertionError(f"OpenML regression dataset name drift: {dataset.name} != {expected_name}")

    X,y,_,_=dataset.get_data(target=task.target_name,dataset_format="dataframe")
    X=X.reset_index(drop=True); y=y.reset_index(drop=True)
    if not pd.api.types.is_numeric_dtype(y):
        # Numeric coercion is allowed only after source selection; no future split determines it.
    ...
    target_threshold,labels=training_median_binary_target(ys,train)
    ...
    for feature in range(len(feature_names)):
        ...
        descriptors=_descriptors(train_values,train_labels,f"openml-regression-task-{task_id}",feature)
    ...
    if adaptive_accept:
        ...
        provenance=hashlib.sha256(
            (f"openml-regression-transfer-v1|task={task_id}|data={data_id}|checksum={checksum}|"
    ...
    return {
      ...
      "target_threshold_source":"training_split_only","task_family":"regression"
    }

def main():
    freeze_manifest()
    manifest=json.loads(Path("openml-regression-transfer-v1-manifest.json").read_text())
    if manifest["target_data_downloaded"] or manifest["manifest_digest"]!=MANIFEST_SHA:
        raise AssertionError("regression transfer manifest drift or leakage")
    if manifest.get("task_type")!="Supervised Regression":
        raise AssertionError("regression task family drift")
    ...
    acquisition_policy=policy_from_memory(memory)
    ...
    worlds=[load_regression_world(
      int(m["task_id"]),int(m["data_id"]),m["name"],acquisition_policy,budget_policy
    ...
    result={
      "manifest_digest":MANIFEST_SHA,"task_family":"regression",
      "target_transform":"binary consequence is y > median(training target); threshold never uses calibration or deployment targets",
      ...
      "worlds":[{
        ...
        "fixed_verified":fixed["source_stats"][w["task_id"]]["verified"],
        "adaptive_verified":adaptive["source_stats"][w["task_id"]]["verified"],
        "retained_verified":retained["source_stats"][w["task_id"]]["verified"],
      } for w in worlds],
    }

    gates={
      "twenty_source_only_regression_worlds":len(worlds)==20,
      "all_target_thresholds_train_only":all(w["target_threshold_source"]=="training_split_only" for w in worlds),
      "nontrivial_acquisition":sum(1 for w in worlds if w["adaptive_accept"])>=6,
      ...
      "lifecycle_reduction_at_least_8x":lifecycle>=8.0,
    }
    result["gates"]=gates; result["passed"]=all(gates.values())
    Path(os.environ.get("REALITYGRAPH_OPENML_REGRESSION_RESULT","openml-regression-transfer-v1-summary.json")).write_text(
      json.dumps(result,sort_keys=True,indent=2)+"\n"
    )

    print("REALITYGRAPH / CROSS-FAMILY REGRESSION DEVELOPMENTAL TRANSFER V1")
    print("---------------------------------------------------------------")
    print(f"manifest_digest={MANIFEST_SHA}")
    print("target_threshold_source=training_split_only")
    ...
    print("VERDICT")
    if result["passed"]:
        print("PASS_CROSS_FAMILY_REGRESSION_DEVELOPMENTAL_TRANSFER_V1")
    else:
        print("PARTIAL_CROSS_FAMILY_REGRESSION_DEVELOPMENTAL_TRANSFER_V1")
        raise AssertionError("one or more frozen regression transfer gates failed")
