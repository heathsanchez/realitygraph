...
SEED="realitygraph-capability-selective-representation-v4-source-only"
...
def main():
    ...
    req=urllib.request.Request(PMLB,headers={"User-Agent":"RealityGraph selective representation V4 manifest"})
    ...
    generator_v2_names={stem(x) for x in (
        "bengaluru_real_estate_price","quake","UHPC-tmp","water-treatment",
        "law-school-admission-bianry","compass","yacht_hydrodynamics",
        "UCI-student-performance-por","colrec","naval_propulsion_plant",
        "Another-Dataset-on-used-Fiat-500-(1538-rows)","IoT_Agriculture_2024",
        "Riga-real-estate-dataset","tumor","climate_insights_dataset",
        "plasma_retinol","bkd1kmc1","aids2","NHANES_age","cholesterol"
    )}
    controller_v3_names={stem(x) for x in (
        "Intersectional-Bias-Assessment-(Training-Data)","datapm2.5",
        "synchronous_machine","women_clothing_review","uis","audit-data",
        "mauna-loa-atmospheric-co2","e1684","Job_Profitability","gbsg",
        "dataset_sales",
        ...
        "channing","sulfur","NASA_PHM2008","fourclass","laser",
        "Minneapolis-Air-Quality-Survey","physicochemical-protein",
    ...
    for _,x in tasks.iterrows():
        ...
        if s in pmlb_names or s in v1names or s in v2names or s in generator_v1_names or s in generator_v2_names or s in controller_v3_names: continue
    ...
    Path("capability-selective-representation-v4-manifest.json").write_text(json.dumps(out,sort_keys=True,indent=2)+"\n")
