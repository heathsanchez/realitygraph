...
def _b_positive() -> dict[str, Any]:
    return {
        "treatment": list(_TREATMENT),
        "wrong_class": list(_WEAK),
        "shuffled_coupling": list(_WEAKER),
        "acquisition_posterior_target_bisimulation_bayes": list(_WEAK),
        "intervention_agreement": [1] * 240 + [0] * 16,
        "hard_gates": {
...
def _g_positive() -> dict[str, Any]:
    ...
    return {
        "pairs": pairs,
        "max_dose_relevant": [1] * _N,
        "max_dose_irrelevant": [0] * _N,
        "hard_gates": {
            "preclassified": True,
...
def qualification_fixtures() -> tuple[QualificationFixture, ...]:
    ...
    # A: planted growth and ordinary explanations.
    a = _a_positive()
    fixtures.append(
        _fixture(
            "A_PLANTED_GROWTH",
    ...
    # B: planted transfer and ordinary target/posterior explanations.
    b = _b_positive()
    fixtures.append(
        _fixture(
            "B_PLANTED_TRANSFER",
    ...
    b_bisim = deepcopy(b)
    b_bisim["acquisition_posterior_target_bisimulation_bayes"] = list(_TREATMENT)
    fixtures.append(
        _fixture(
    ...
    b_post = deepcopy(b)
    b_post["acquisition_posterior_target_bisimulation_bayes"] = list(_TREATMENT)
    fixtures.append(
        _fixture(
    ...
    b_shuffle["shuffled_coupling"] = list(_TREATMENT)
    fixtures.append(
        _fixture(
    ...
    # G: planted dose-response, ordinary nulls, and invalid post-hoc labels.
    g = _g_positive()
    fixtures.append(
        _fixture(
    ...
    g_null["max_dose_relevant"] = [0] * _N
    g_null["max_dose_irrelevant"] = [0] * _N
    fixtures.append(
        _fixture(
    ...
    g_damage = deepcopy(g_null)
    g_damage["pairs"] = [
        {"weight": weight, "relevant": 1, "irrelevant": 1}
        for weight in (2, 5, 10, 20)
        for _ in range(16)
    ]
    g_damage["max_dose_relevant"] = [1] * _N
    g_damage["max_dose_irrelevant"] = [1] * _N
    fixtures.append(
        _fixture(
    ...
    g_reverse["pairs"] = [
        {"weight": weight, "relevant": 0, "irrelevant": 1}
        for weight in (2, 5, 10, 20)
        for _ in range(16)
    ]
    g_reverse["max_dose_relevant"] = [0] * _N
    g_reverse["max_dose_irrelevant"] = [1] * _N
    fixtures.append(
        _fixture(
    ...
    # P: planted persistence and ordinary no-retention explanations.
    p = _p_positive()
    fixtures.append(
        _fixture(
