...
This is the intended empirical loop:

    enormous lawful property field
        -> consequence selects distinctions
        -> independent groups verify them
        -> deletion contracts the representation
        -> sealed future decides whether it survives

For the Parkinson imaging use case that motivated this path, the mapping is direct:

    scan              -> empirical state
    image property    -> probe
    property value    -> observation
    pathologic/normal -> consequence
    hospital/acquisition environment -> group
...
CI runs the unit suite and the frozen demos on every push.
