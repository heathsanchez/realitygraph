...
SEED="realitygraph-openml-v16-source-only"
...
def main():
    ...
    req=urllib.request.Request(PMLB,headers={"User-Agent":"RealityGraph V16 manifest"})
    ...
    v15names={stem(x) for x in (
        "visualizing_soil","strikes","vertebra-column",
        "regensburg_pediatric_appendicitis","HMEQ_Data",
        "hepatitis_c_virus_hcv_for_egyptian_patients",
        "polish_companies_bankruptcy","JapaneseVowels",
        "South_Asian_Churn_dataset","arsenic-female-lung","autoHorse",
        "students_scores","waveform-5000","colleges_aaup",
        "Cirrhosis_Patient_Survival_Prediction","eye_movements",
        "Tour-and-Travels-Customer-Churn-Prediction","CostaMadre1",
        "wall-robot-navigation","mammography"
    )}
    bydata={}
    ...
    Path("openml-v16-manifest.json").write_text(json.dumps(out,sort_keys=True,indent=2)+"\n")
