...
SEED="realitygraph-capability-generator-controller-transfer-v3-source-only"
...
def main():
    ...
    req=urllib.request.Request(PMLB,headers={"User-Agent":"RealityGraph capability generator controller transfer V3 manifest"})
    ...
    for _,x in tasks.iterrows():
        ...
        if s in pmlb_names or s in v1names or s in v2names or s in generator_v1_names or s in generator_v2_names: continue
    ...
    Path("capability-generator-controller-transfer-v3-manifest.json").write_text(json.dumps(out,sort_keys=True,indent=2)+"\n")
