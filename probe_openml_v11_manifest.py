...
SEED="realitygraph-openml-v11-source-only"
...
def main():
    ...
    req=urllib.request.Request(PMLB,headers={"User-Agent":"RealityGraph V11 manifest"})
    ...
    for _,x in tasks.iterrows():
        ...
        name=str(x[nc])
        if did in OLD or stem(name) in oldnames or stem(name) in v7names or stem(name) in v8names or stem(name) in v9names or stem(name) in v10names or not(200<=rows<=50000 and 2<=f<=100 and 2<=c<=10 and num>=2):continue
    ...
    Path("openml-v11-manifest.json").write_text(json.dumps(out,sort_keys=True,indent=2)+"\n")
