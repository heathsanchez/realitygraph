...
def regression_manifest(summary_raw: bytes) -> list[dict]:
    ...
    if len(eligible) < 100:
        raise AssertionError(
            "regression eligibility produced "
            f"only {len(eligible)} datasets"
        )
    return eligible[:100]


def _balanced_binary_labels(
    targets: list[float],
) -> tuple[list[int], float]:
    unique = sorted(set(targets))
    if len(unique) < 2:
        raise ValueError(
            "regression target has fewer than "
            "two distinct values"
        )
...
def main():
    index = int(
        os.environ[
            "REALITYGRAPH_PMLB_REGRESSION_INDEX"
        ]
    )
    out_path = Path(
        os.environ.get(
            ...
            (
                "results/"
                f"pmlb-regression-world-{index:03d}.json"
            ),
        )
    )
    ...
    print(
        f"PMLB_REGRESSION_WORLD "
        f"index={index:03d} "
        f"dataset={name} "
        f"rows={len(rows)} "
        f"features={len(anon_names)} "
        f"positive_rate="
