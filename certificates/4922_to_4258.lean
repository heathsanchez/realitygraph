...
-- RealityGraph compiled consequence: Equation4922 → Equation4258
@[reducible] def EquationTarget4258 (G : Type _) [Magma G] : Prop :=
  ∀ (x : G) (y : G) (z : G) (w : G),
    x ◇ y = ((z ◇ w) ◇ w) ◇ y

abbrev Goal4258 : Prop :=
  ∀ (G : Type) [Magma G], EquationLHS G → EquationTarget4258 G

def submission_4922_to_4258 : Goal4258 := by
  intro G _ h
  intro x y z w
  have derived : EquationRHS G := submission G h
  exact derived x y (z ◇ w) w

theorem certificate_4922_to_4258 :
    ∀ (G : Type) [Magma G], EquationLHS G → EquationTarget4258 G :=
  submission_4922_to_4258

#print axioms certificate_4922_to_4258
